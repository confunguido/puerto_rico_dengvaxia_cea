...
Surveillance::Surveillance(){
    ...
    reportTodayProb = 0.0;
    printExposure = false;
    vaxSecondaryLikeInfection = true; // By default vaccine acts as secondary infection in terms of viremia
    enableTempImmunity = true; // By default enable temporary immunity within the trial
...
void Surveillance::setup(string file){
    ...
    vaxSecondaryLikeInfection = this->readParameter("surveillance_vax_secondary_like_infection",vaxSecondaryLikeInfection);
    enableTempImmunity = this->readParameter("surveillance_enable_temp_immunity",enableTempImmunity);
    ...
    if(vaxSecondaryLikeInfection){
      printf("Vaccinated infections have secondary-like viremia\n");
    }else{
      printf("Vaccinated infections have primary-like viremia\n");
    }
    if(enableTempImmunity){
      printf("TEMPORARY IMMUNITY is ENABLED\n");
    }else{
      printf("TEMPORARY IMMUNITY IS DISABLED\n");
    }
}
...
void Surveillance::initialize_human_surveillance(Human * h, int currDay){
    ...
    h->setEnableTempImmunity(enableTempImmunity);
...
int Surveillance::update_human_surveillance(Human * h, int currDay, RandomNumGenerator * rGen){
	...
	if( ( currDay - recordsDatabase.find(id)->second.enrollmentDay ) >= 30){
		...
		if(h->getReportSymptoms()){
		    if(currDay > recordsDatabase.find(id)->second.onset[serotype] + 2 && currDay <= recordsDatabase.find(id)->second.onset[serotype] + 5){
		    ...
		    }else if(currDay > recordsDatabase.find(id)->second.onset[serotype] + 5){
...
int Surveillance::PCR_test(Human * h, int currDay, RandomNumGenerator * rGen){
	...
	// Vaccinees will have a secondary-like viral curve when surveillance_secondary_like_infection is enabled
	double b1 = 0.0;
	double b2 = 0.0;
	if(h->infection->isPrimary() == false || (h->isVaccinated() == true && vaxSecondaryLikeInfection == true)){
	  // secondary infection
	    b1 = 6.834631;
	    b2 = -1.166282;
	}else{
