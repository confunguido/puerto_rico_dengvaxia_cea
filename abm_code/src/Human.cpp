	     ...
	     RandomNumGenerator& rGen)
{
    ...
    vaccinated = false;
    testedPreviousExposure = false;
    dead = false;
    resetRecent();
    cohort = 0;
    cohortCounted = false;
    popCounted = false;
    seroprevalenceCounted = false;
    tAge = 0;
    vday = -1;
    trialDay = 0;
    vaxWaning_pos = 0;
...
void Human::checkRecovered(unsigned currDay){
    if((unsigned) infection->getEndDay() <= currDay){
       infection.reset(nullptr);
       infected = false;
       hospitalized = false;
       willDieFromInfection = false;
    ...
    string infLocID_)
{
	...
	// There are multiple vaccines supported: GSK, advance (Sanofi), age, or simple
	...
	}else if(vaccineProfile.getMode() == "simple"){
	    RRInf = vaccineProfile.getRRInf(getPreviousInfections() > 0,infectionType - 1);
	    RRDis = vaccineProfile.getRRDis(getPreviousInfections() > 0, infectionType - 1);
    ...
    if(isImmuneVax() == true && vaccineProfile.getMode() == "advance"){
        vax_protection = 1.0 - vaccineProfile.getVaccineProtection(getPreviousInfections() > 0);
    }
	...
	R0[infectionType] = 0;
        if(vaccinated){
            printf("Infecting human VACCINE with profile %s...age: %d\n",vaccineProfile.getMode().c_str(),getAgeDays(currentDay));
        }
	if(humInf != nullptr){
	...
	if(hospitalized == true){
	    if(rGen->getEventProbability() < 0.005){
		willDieFromInfection = true;
	    }
	}
	preExposureAtVaccination[infectionType - 1] = getPreviousInfections();
	
	for(unsigned int i = 0;i < N_SERO; i++){
...
void Human::vaccinateAdvanceMode(int currDay, RandomNumGenerator& rGen)
{
    ...
    double vax_wan = vaccineProfile.getWaning(getPreviousInfections() > 0);
    setVaxImmEndDay(currDay + rGen.getVaxHumanImmunity(rGen.getWaningTime(vax_wan)));
}
...
void Human::vaccinateWithProfile(int currDay, RandomNumGenerator * rGen, Vaccine  vax){
	...
	}else if(vaccineProfile.getMode() == "placebo"){
	    this->vaccinate(currDay);
	}
...
void Human::updateVaccineEfficacy(unsigned int currDay){
