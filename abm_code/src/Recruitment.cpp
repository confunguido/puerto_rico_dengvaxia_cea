...
void Recruitment::updateArm(unsigned vaxID, recruit_t & arm, unsigned currDay){
	...
	for(auto it = arm.begin(); it != arm.end(); ){
		...
		if(phum->isEnrolledInTrial() == true){
		    ...
		    if(( currDay < (unsigned int) (phum->getTrialEnrollmentDay() + trialMaximumDays) ) && (currDay < (unsigned int) (phum->getTrialEnrollmentDay() + trialDurationDays) ||  (unsigned int) pcr_cases < trialMinimumCases)){
		      if(currDay > (unsigned int) (recruitmentStartDay + recruitmentTimeFrame) && rGen->getEventProbability() < dropoutRate){
				...
				it = arm.erase(it);
			        ...
			        if(phum->isFullyVaccinated() == false && (unsigned int) phum->getNextDoseDay() == currDay){
